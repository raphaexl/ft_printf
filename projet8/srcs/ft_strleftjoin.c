...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/06 14:33:00 by ebatchas          #+#    #+#             */
/*   Updated: 2019/03/06 20:52:54 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_handle_precision(char *buffer, int prec)
{
	...
	if (i <= 0)
	{
		...
		temp = ft_strsub(buffer, 0, len + 1);
		return (ft_strjoin(temp, ft_strsub(p + 1, 0, 6)));
	}
	temp = (char *)malloc(sizeof(char) * (i + 1));
	while (--i >= 0)
